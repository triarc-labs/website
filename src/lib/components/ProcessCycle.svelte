...
<div role="presentation" on:mouseenter={() => (hovering = true)} on:mouseleave={() => (hovering = false)}>
      ...
      >
        <span
          ...
          index
            ...
            : 'border-transparent bg-black text-white group-hover:bg-gray-800'}"
        >
          {index + 1}
        </span>
  ...
  <div class="mt-6 min-h-[200px] md:min-h-[160px]">
    {#key active}
      ...
      >
        <p class="max-w-3xl text-lg text-black">{steps[active].content}</p>
      </div>
    {/key}
  </div>
  <p class="mt-6 flex items-center gap-x-3 text-base text-black">
