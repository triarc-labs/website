...
>
  <Container>
    <div class="py-20 md:py-28">
      ...
      <Reveal y={20}>
        <div class="max-w-3xl">
          <span class="text-sm font-bold uppercase tracking-widest text-white/70">{content.kicker}</span>
          <h2 class="mt-3 text-3xl font-bold leading-tight tracking-tight md:text-4xl md:leading-tight">
            {content.title}
          </h2>
          <div class="mt-6 space-y-4">
            {#each content.roleParagraphs as paragraph}
              <p class="text-lg text-white/90">{paragraph}</p>
            {/each}
          </div>
        </div>
      </Reveal>
      ...
      <Reveal y={20} delay={120}>
        <div class="mt-12 md:mt-16">
          <h3 class="mb-8 text-sm font-bold uppercase tracking-widest text-white/70">{content.flowTitle}</h3>
          ...
          <div class="hidden md:grid md:grid-cols-4 md:gap-x-6" aria-hidden="true">
            ...
            {#each content.phases as phase, i}
              <div class="text-center">
                <div
                  class="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-white text-lg font-bold text-red-triarc"
                >
                  {i + 1}
                </div>
                <div class="mt-3 text-base font-bold leading-snug text-white">{phase.name}</div>
                <div class="mx-auto mt-3 h-4 w-px bg-white/30"></div>
              </div>
            {/each}

            <!-- Row 2: the Tech-Lead spine spanning every phase -->
            <div class="col-span-4">
              <div class="relative min-h-[2.5rem] overflow-hidden rounded-full bg-white shadow-sm">
                <!--
                ...
                <div class="pointer-events-none absolute inset-0 grid grid-cols-4 gap-x-6">
                  <div class="tech-lead-pill flex items-center justify-center">
                    <span
                      class="inline-flex items-center rounded-full bg-red-triarc px-3 py-1 text-xs font-bold uppercase tracking-wide text-white shadow-sm"
                    >
                      {content.leadLabel}
                    </span>
                  </div>
            ...
            {#each content.phases as phase}
              <div class="text-center">
                <div class="mx-auto h-4 w-px bg-white/30"></div>
                <div class="mt-3 flex flex-wrap justify-center gap-1.5">
                  {#each phase.collaborators as collaborator}
                    <span
                      class="rounded-full bg-white/15 px-2.5 py-1 text-xs font-semibold text-white ring-1 ring-white/20"
                    >
                      {collaborator}
                    </span>
                  {/each}
                </div>
                <p class="mx-auto mt-3 max-w-[26ch] text-sm leading-relaxed text-white/85">{phase.description}</p>
              </div>
            {/each}
          </div>
          ...
          <div class="md:hidden" aria-hidden="true">
            <p class="mb-5 flex items-center gap-2 text-sm font-semibold text-white">
              <span
                class="inline-flex items-center rounded-full bg-white px-3 py-1 text-xs font-bold uppercase tracking-wide text-red-triarc"
              >
                {content.leadLabel}
              </span>
              durchgehend dabei
              ...
              {#each content.phases as phase, i}
                <li class="relative flex gap-4 pb-8 last:pb-0">
                  <div class="flex flex-col items-center">
                    <div
                      class="z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white text-base font-bold text-red-triarc"
                    >
                      {i + 1}
                    </div>
                    {#if i < content.phases.length - 1}
                      <div class="mt-1 w-0.5 flex-1 bg-white/30"></div>
                    {/if}
                  </div>
                  <div class="flex-1 pb-1">
                    <div class="text-lg font-bold text-white">{phase.name}</div>
                    <p class="mt-1 text-sm leading-relaxed text-white/85">{phase.description}</p>
